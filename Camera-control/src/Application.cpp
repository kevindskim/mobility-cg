...
int main(void)
{
	...
	{ 
		...
		//
		// 	Camera(glm::vec3 initEye, glm::vec3 initUp,float initYaw, float initPitch,float initMoveSpeed, float initTurnSpeed);
		//  yaw 값이 0일때는 front가 [1,0,0]이므로, yaw를 -90으로 해서 초기 front가 [0,0,-1]이 되도록 함
		
		Camera camera{ glm::vec3{0.0f,0.0f,5.0f}, glm::vec3{0.0f,1.0f,0.0f}, -90.0f, 0.0f, 2.5f, 0.1f };
		...
		while (!mainWindow.GetShouldClose())
		{
			...
			// proramg closing logic goes here with escape key
