...
int main(void)
{
	...
	{ 
		...
		// Camera(initEye, initUp, initYaw, initPitch, initMoveSpeed, initTurnSpeed)
		// yaw 값이 0일때는 front가 [1,0,0]이므로, yaw를 -90으로 해서 초기 front가 [0,0,-1]이 되도록 함
		// initMoveSpeed는 Key 조작에 따른 카메라의 이동 속도, initTurnSpeed는 마우스 움직임에 따른 회전 속도

		Camera camera{ glm::vec3{0.0f,0.0f,15.0f}, glm::vec3{0.0f,1.0f,0.0f}, -90.0f, 0.0f, 5.0f, 0.2f };
		...
		while (!mainWindow.GetShouldClose())
		{
			...
			//좌측 컨트롤 키를 누르고 마우스를 움직이면 카메라가 회전하도록 함			
			isControlPressed = mainWindow.GetKeys()[GLFW_KEY_LEFT_CONTROL];
			if (isControlPressed) {
