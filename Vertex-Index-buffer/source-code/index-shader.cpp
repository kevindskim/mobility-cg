...
int main(void)
{
	...
	glGenBuffers(1, &vbo); //1. 버텍스 버퍼 생성
	...
	glEnableVertexAttribArray(0); //1. 몇 번째 location의 attribute를 활성화(enable) 할 것인지 전달 (여기서는 0번째) 
	                              // basic.shader 파일내 vertex shader에정의 됨. layout(location = 0) in vec4 position; 
	...
	//---------Shader 생성---------------//
	ShaderProgramSource source = ParseShader("resource/shaders/Basic.shader"); //셰이더 코드 파싱
