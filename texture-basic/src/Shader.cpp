	...
	:m_FilePath{ filepath }, m_RendererID{ 0 }
{
	...
	m_RendererID = CreateShader(source.VertexSource, source.FragSource);  //셰이더 프로그램 생성, m_RendererID is program id
}
...
unsigned int Shader::CompileShader(unsigned int type, const std::string& source)
{
	...
	if (result == GL_FALSE) //컴파일에 실패한 경우
	{
		...
		char* message = (char*)malloc(length * sizeof(char)); //stack에 동적할당
		if (message == nullptr) {
			std::cout << "Failed to allocate memory for shader error log" << std::endl;
			glDeleteShader(id); //메모리 할당 실패한 경우 셰이더 삭제
			return 0;
		}
...
unsigned int Shader::CreateShader(const std::string& vertexShader, const std::string& fragShader)
{
	...
	// Link Error Handling(added by DS on Aug 3, 2024)
	int result;
	glGetProgramiv(program, GL_LINK_STATUS, &result); //셰이더 프로그램으로부터 컴파일 결과(log)를 얻어옴
	if (result == GL_FALSE) //링크에 실패한 경우
	{
		int length;
		glGetProgramiv(program, GL_INFO_LOG_LENGTH, &length); //log의 길이를 얻어옴

		char* message = (char*)malloc(length * sizeof(char)); //stack에 동적할당
		if (message == nullptr) {
			std::cout << "Failed to allocate memory for link error log" << std::endl;
			glDeleteProgram(program); //메모리 할당 실패한 경우 프로그램 삭제
			return 0;
		}

		glGetProgramInfoLog(program, length, &length, message); //길이만큼 log를 얻어옴
		std::cout << "링크 실패! " << std::endl;
		std::cout << message << std::endl;

		free(message); //메모리 동적할당 해제
		glDeleteProgram(program); //링크에 실패한 경우 프로그램 삭제
		return 0;
	}
	//
