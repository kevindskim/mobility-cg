...
struct VertexBufferElement
{
	...
	static unsigned int GetSizeOfType(unsigned int type) //타입별로 적절한 메모리 사이즈를 반환하는 static 함수, static 함수는 구조체의 인스턴스 없이 호출될 수 있는 함수.
	{
		switch (type)
		{
		...
		ASSERT(0);  //위의 타입이 아닌 경우 에러, 0는 false로 평가
		return 0;
	}
};
